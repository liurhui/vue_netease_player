    ...
    <div class="oth clearfix" v-show="area != -1">
        ...
        <div class="moreOther clearfix">
          <li class="sml" v-for="sml,index in artlist2" :key="index">
             <router-link :to="{path:'/artist/song',query:{id:sml.id}}" class="over underline">{{sml.name}}</router-link>              <i class="pointer" v-if="sml.alias.length != 0"></i>
          </li>
      </div>
    </div>
  </div>
...
export default {
    ...
    methods:{
        ...
        async getArtistTop(limit){
            ...
            this.allartlist = result.artists;
        },
        //获取分类歌手
