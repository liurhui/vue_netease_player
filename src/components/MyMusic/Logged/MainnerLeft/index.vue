<template>
  <div class="mainnerLeft">
      <div class="wrapper clearfix">
          <h2 class="usersub hover">我的歌手&nbsp;&nbsp;(1)</h2>
          <!-- 创建的歌单 -->
    ...
    methods:{
        ...
        async getPlayList(){
            ...
            result.playlist.forEach((item)=>{
                if(item.userId == localStorage.getItem("USERID")) this.myList.push(item),this.myListNum++;
                else this.likeList.push(item),this.likeListNum++;
            })
            this.myList[0].name = '我喜欢的音乐';
        },
        //显示与隐藏我创建的歌单
        showMyList(){
